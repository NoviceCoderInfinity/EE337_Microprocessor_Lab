...
void main(void)
{
	...
	LCD_Init();
	Timer_Init();
	LCD_StringWrite("Weight: xxxx gm", 15);
	...
	while(1)												// endless 
	{
		while(take_samples)
		{
			...
			avgVal = avgVal/10.0;					// the average reading
			avgVal = avgVal*(3700.0/1024.0); // in mV
			avgVal = avgVal*0.83 + 3.42 // corrextion based on the obserations in the readings.ods file
			LCD_CmdWrite(0x88);						// move the cursor to first line, ninth position
			weight[3] = avgVal%10 + 0x30; // add 0x30 to convert the corresponding number to ASCII
			avgVal = avgVal/10;
			weight[2] = avgVal%10 + 0x30;
			avgVal = avgVal/10;
			weight[1] = avgVal%10 + 0x30;
			avgVal = avgVal/10;
			weight[0] = avgVal%10 + 0x30;
