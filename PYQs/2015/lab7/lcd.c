...
void main(void)
{
	...
	while(1)			   							// endless  
  {
		...
		delay_ms(5000);
		LCD_CmdWrite(0x80);					// Put cursor on first row, 0 column
		sdelay(18);									// Delay of 270us
		LCD_WriteString("Hello", 5);	// Write "Hello" in the first line
		sdelay(18);
		LCD_CmdWrite(0xCB);					// Put cursor on second row, 11 column
		sdelay(18);
		LCD_WriteString("World", 5);	// Write "World" on the second line
	}
}
